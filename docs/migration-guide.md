...
The Forge-specific `/v1/forge/*` endpoints are opt-in. If you never call them, Forge runs as a plain OpenAI-compatible inference server. The only visible difference in existing clients is the `x_forge` field appended to each response — OpenAI SDK clients silently ignore unknown fields.
...
**Economy**: Ollama has no economic layer. On Forge, every inference call records a trade and charges CU. New nodes start with a 1,000 CU welcome loan (0% interest, 72-hour term per parameters.md §3), so you begin with credit.

---

## From mesh-llm (upstream fork)

Forge is a strict superset of mesh-llm. The L0 inference layer (iroh QUIC, Noise encryption, pipeline parallelism, MoE sharding, Nostr peer discovery) is inherited directly. Forge adds L1–L4 (CU economy, forge-bank, forge-mind, forge-agora) on top without removing anything from L0.
...
# CU accounting starts on the first inference call.
# Welcome loan = 1,000 CU at 0% interest (parameters.md §3).
```

The `nm-arealnormalman/mesh-llm` fork is kept at Phase 10 parity with `clearclown/forge` — both expose the same 45 economic endpoints under `/v1/forge/*` and `/api/forge/*` respectively. Use whichever directory layout feels more natural. For new deployments, `clearclown/forge` is the recommended entry point.
...
| | Bittensor | Forge |
|---|---|---|
| Currency | TAO (ERC-20-style token) | CU (unit of account, not a token) |
...
| Registration fee | Yes (TAO to register on subnet) | No (welcome loan covers first 1,000 CU free) |
...
If you were running a Bittensor miner to earn TAO, the Forge equivalent is `forge seed -m <model>`. Start the binary, and you begin earning CU on the first request served. No wallet registration, no TAO stake, no subnet approval.
...
The `forge-ledger.json` file is a JSON record of your CU trade history. It is human-readable and worth keeping as an audit log even after decommissioning the node.
