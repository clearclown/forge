...
每个响应都包含 `x_forge` — **该计算的 CU 成本**以及剩余余额。提供者赚取了 9 CU，消费者花费了 9 CU。物理学为每个单位提供了支撑。
...
每次推理都是一笔交易。提供者赚取 CU，消费者支出 CU。没有区块链，没有代币，没有 ICO。CU 由物理学支撑——即为有用工作而消耗的电力。与 Bittensor (TAO)、Akash (AKT) 或 Golem (GLM) 不同，CU 无法被投机——它通过执行有用计算来赚取。
...
节点可以将闲置 CU 以利息借给其他节点。小型节点借入 CU，访问更大的模型，赚取更多 CU，并支付利息偿还。没有其他分布式推理项目提供计算借贷。这是让自我改进循环对每个人（而不仅仅是那些已经拥有强大硬件的人）在经济上都可行的引擎。

## 架构

```
┌─────────────────────────────────────────────────┐
│  L4: 发现 (forge-agora) ✅ v0.1                 │
│  代理市场、声誉聚合、                             │
...
此脚本会从 HuggingFace 下载 SmolLM2-135M（约 100 MB），启动带有 Metal/CUDA 加速的真实 Forge 节点，运行三次真实的聊天完成，遍历第 1-12 阶段的所有端点，并打印彩色摘要。已于 2026-04-09 在 Apple Silicon Metal GPU 上验证。
...
[PyPI: forge-sdk](https://pypi.org/project/forge-sdk/)（20 个 L2/L3/L4 方法）·
[PyPI: forge-cu-mcp](https://pypi.org/project/forge-cu-mcp/)（为 Claude Code / Cursor 提供 20 个 MCP 工具）
...
# 或以下任意命令：
./target/release/forge chat -m "smollm2:135m" "什么是重力？"
./target/release/forge seed -m "qwen2.5:1.5b"               # 作为 P2P 提供者赚取 CU
./target/release/forge worker --seed <public_key>           # 作为 P2P 消费者花费 CU
./target/release/forge models                                # 列出目录（或使用 HF URL / 简写）
```
...
预编译二进制文件和 `clearclown/forge:latest` Docker 镜像在
...
forge/  (本仓库 — 第 1 层)
├── crates/
│   ├── forge-ledger/      # CU 核算、借贷、agora (NIP-90)、安全
│   ├── forge-node/        # 节点守护进程、HTTP API（借贷 + 路由）、流水线
│   ├── forge-cli/         # CLI: 聊天、播种、工人、结算、钱包
│   ├── forge-lightning/   # CU ↔ 比特币 Lightning 桥接（双向）
│   ├── forge-net/         # P2P: iroh QUIC + Noise + gossip（交易 + 贷款）
│   ├── forge-proto/       # 线缆协议: 27+ 种消息类型，含 Loan*
│   ├── forge-infer/       # 推理引擎: llama.cpp, GGUF, Metal/CPU
│   ├── forge-core/        # 类型定义: NodeId, CU, Config
│   └── forge-shard/       # 拓扑: 层级分配
├── sdk/python/forge_sdk.py        # 带完整借贷 API 的 Python 客户端
├── mcp/forge-mcp-server.py        # MCP 服务器（面向 Claude 等的借贷工具）
├── scripts/verify-impl.sh         # TDD 回归测试（24 个断言）
└── docs/                  # 规范、战略、威胁模型、路线图
```

约 14,500 行 Rust。**143 个测试通过。** 第 1-6 阶段完成。

## 姊妹仓库（完整生态系统）

| 仓库 | 层级 | 测试数 | 状态 |
|------|-------|-------|--------|
| [clearclown/forge](https://github.com/clearclown/forge)（本仓库） | L1 经济 | 143 | 第 1-6 阶段 ✅ |
...
| [clearclown/forge-agora](https://github.com/clearclown/forge-agora) | L4 发现 | 39 | v0.1 ✅ |
| [clearclown/forge-economics](https://github.com/clearclown/forge-economics) | 理论 | 16/16 GREEN | ✅ |
...
- [货币理论](../../../docs/monetary-theory.md) — 为什么 CU 可行：Soddy、比特币、PoUW、仅限 AI 的货币
- [概念与愿景](../../../docs/concept.md) — 为什么计算即金钱
- [经济模型](../../../docs/economy.md) — CU 经济、有用工作证明、借贷
...
- [A2A 支付](../../../docs/a2a-payment.md) — 面向代理协议的 CU 支付扩展
