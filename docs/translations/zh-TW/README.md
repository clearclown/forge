...
節點可以將閒置 CU 以利息借給其他節點。小型節點借入 CU，存取更大的模型，賺取更多 CU，並支付利息還款。沒有其他分散式推理專案提供計算借貸——透過對該領域所有主要專案的競爭分析已得到確認。這是讓自我改進迴圈對每個人（而不僅僅是那些已經擁有強大硬體的人）在經濟上都可行的引擎。
...
- [策略](strategy.md) — 競爭定位、借貸規範、5 層架構
- [貨幣理論](monetary-theory.md) — 為什麼 CU 可行：Soddy、比特幣、PoUW、僅限 AI 的貨幣
- [概念與願景](concept.md) — 為什麼計算即金錢
- [經濟模型](economy.md) — CU 經濟、有用工作證明、借貸
- [架構設計](architecture.md) — 雙層設計
- [代理整合](agent-integration.md) — SDK、MCP、借貸工作流程
- [線纜協定](protocol-spec.md) — 17 種消息類型
- [路線圖](roadmap.md) — 開發階段
- [威脅模型](threat-model.md) — 安全 + 經濟攻擊
- [引導啟動](bootstrap.md) — 啟動、降級、恢復
- [A2A 支付](a2a-payment.md) — 面向代理協定的 CU 支付擴充
