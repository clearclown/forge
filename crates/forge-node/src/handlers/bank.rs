//! HTTP handlers for `/v1/forge/bank/*` endpoints (Phase 8 / Batch B1).
//!
...
/// POST /v1/forge/bank/tick — run one strategy tick against the current ledger pool
...
/// POST /v1/forge/bank/strategy — hot-swap the portfolio strategy
pub(crate) async fn bank_set_strategy(
    ...
    let new_strategy: Box<dyn forge_bank::Strategy> = match req.strategy.as_str() {
...
/// POST /v1/forge/bank/risk — set the risk tolerance
...
/// POST /v1/forge/bank/futures — create a new FuturesContract
pub(crate) async fn bank_create_futures(
    ...
    let margin_cu = if let Some(frac) = req.margin_fraction {
        ...
        (req.notional_cu as f64 * frac).floor() as u64
    } else {
        (req.notional_cu as f64 * 0.10).floor() as u64
    };
...
/// GET /v1/forge/bank/risk-assessment
pub(crate) async fn bank_risk_assessment(
    State(state): State<AppState>,
) -> Result<Json<forge_bank::RiskAssessment>, (StatusCode, String)> {
    check_forge_rate_limit(&state).await?;
    let bank = state.bank.lock().await;
...
pub(crate) async fn bank_optimize(
    ...
    let portfolio_value = {
        let bank = state.bank.lock().await;
        (bank.portfolio.portfolio.cash_cu + bank.portfolio.portfolio.total_lent()).max(1)
    };
...
mod tests {
    ...
    async fn test_bank_portfolio_returns_initial_state() {
        ...
        assert!(json["cash_cu"].is_u64());
        assert_eq!(json["cash_cu"].as_u64().unwrap(), 10_000);
    ...
    async fn test_bank_risk_assessment_returns_data() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/bank/risk-assessment")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
