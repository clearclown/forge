//! /v1/forge/anchor — build an OP_RETURN anchor for the current trade Merkle root.
...
use bitcoin::Network;
use forge_ledger::anchor::AnchorRequest;
    ...
    async fn test_anchor_endpoint_returns_payload() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/anchor?network=testnet")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn test_anchor_endpoint_rejects_unknown_network() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/anchor?network=dogecoin")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
