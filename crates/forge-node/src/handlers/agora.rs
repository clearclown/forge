//! HTTP handlers for `/v1/forge/agora/*` endpoints (Phase 8 / Batch B2).
//!
...
use forge_agora::{AgentProfile, CapabilityMatch, CapabilityQuery, RegistrySnapshot, ReputationScore};
...
pub struct FindRequest {
    ...
    pub max_cu_per_token: Option<u64>,
    pub tier: Option<forge_agora::ModelTier>,
...
pub(crate) async fn agora_find(
    ...
    let query = CapabilityQuery::new(
        req.model_patterns,
        req.max_cu_per_token.unwrap_or(u64::MAX),
        req.tier,
        min_rep,
    )
...
pub(crate) async fn agora_restore(
    ...
    let mut mp = state.marketplace.lock().await;
    mp.registry = forge_agora::AgentRegistry::restore(snapshot);
    ...
    async fn test_agora_reputation_unknown_agent_returns_new_agent_score() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!("/v1/forge/agora/reputation/{hex}"))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
