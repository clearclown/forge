...
mod security_tests {
    ...
    /// POST /v1/forge/bank/strategy with syntactically invalid JSON body → 400.
    ///
    ...
    /// POST /v1/forge/bank/risk with syntactically invalid JSON body → 400.
    ///
    ...
    /// POST /v1/forge/agora/register with an `agent_hex` containing non-hex characters.
    ///
    ...
    /// POST /v1/forge/agora/register where `agent_hex` is only 4 characters.
    ///
    ...
    /// GET /v1/forge/agora/reputation/<path-traversal> — the path parameter is
    ...
    async fn test_reputation_endpoint_handles_path_traversal_attempt() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/agora/reputation/..%2F..%2Fetc%2Fpasswd")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    /// GET /v1/forge/collusion/<non-hex path> — the {hex} segment must be
    ...
    async fn test_collusion_endpoint_handles_non_hex_path() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/collusion/DROP%20TABLE%20nodes")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    /// GET /v1/forge/anchor?network=<xss-payload> — the network query param must
    ...
    async fn test_anchor_endpoint_rejects_xss_in_network_param() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/anchor?network=%3Cscript%3Ealert(1)%3C%2Fscript%3E")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    /// POST /v1/forge/lend with amount=0 → 400 (enforced by handler guard).
    #[tokio::test]
    ...
    /// POST /v1/forge/lend with amount=u64::MAX — documents an integer overflow bug.
    ///
    /// VULNERABILITY DOCUMENTED: `ComputeLedger::can_afford()` uses `cu_cost as i64`
    ...
    /// POST /v1/forge/borrow where term_hours is negative in the JSON payload.
    ...
    /// POST /v1/forge/mind/improve with n_cycles=999999999 → 400 (capped at 100).
    #[tokio::test]
    ...
    /// POST /v1/forge/mind/improve with n_cycles=0 → 400 (must be ≥ 1).
    #[tokio::test]
    ...
    /// POST /v1/forge/bank/futures with notional_cu=0 → 400 (FuturesContract::new
    ...
    /// GET /v1/forge/trades?limit=999999999 → must not OOM; the handler caps at 100.
    #[tokio::test]
    async fn test_trades_handles_huge_limit_without_oom() {
        ...
        let resp = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri("/v1/forge/trades?limit=999999999")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    /// Firing 50+ rapid requests at /v1/forge/balance must eventually produce
    ...
    /// POST /v1/forge/bank/strategy with no Content-Type header → axum should
    ...
    /// POST /v1/forge/bank/strategy with Content-Type: application/xml → 400 or 415.
    #[tokio::test]
    ...
    /// POST /v1/forge/bank/futures where counterparty_hex is fewer than 64 chars → 400.
    #[tokio::test]
    ...
    /// POST /v1/forge/bank/strategy with margin_fraction=0.0 → 400.
    ...
    /// GET /v1/forge/anchor without the network query param uses default "testnet"
    ...
    /// POST /v1/forge/agora/find with min_reputation > 1.0 → 400.
    #[tokio::test]
    ...
    /// POST /v1/forge/agora/find with min_reputation < 0.0 → 400.
    #[tokio::test]
    ...
    /// POST /v1/forge/mind/init with unknown optimizer name → 400.
    #[tokio::test]
    ...
    /// POST /v1/forge/bank/risk with an unknown tolerance string → 400.
    #[tokio::test]
