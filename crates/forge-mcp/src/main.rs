...
impl ForgeMcpServer {
    pub fn new(base_url: String, token: Option<String>) -> Self {
        Self {
            client: Arc::new(ForgeClient::new(base_url, token)),
        }
    }
    ...
    async fn dispatch(&self, name: &str, args: Value) -> CallToolResult {
        ...
        let result: Result<Value, String> = match name {
            ...
            // Economy (6 tools)
            // ----------------------------------------------------------------
            "forge_balance" => self.client.get("/v1/forge/balance").await,
            "forge_pricing" => self.client.get("/v1/forge/pricing").await,
            "forge_trades" => {
                let limit = obj.get("limit").and_then(|v| v.as_i64()).unwrap_or(20);
                self.client
                    .get(&format!("/v1/forge/trades?limit={limit}"))
                    .await
            }
            "forge_network" => self.client.get("/v1/forge/network").await,
            "forge_providers" => self.client.get("/v1/forge/providers").await,
            "forge_inference" => {
                let prompt = obj
                    .get("prompt")
                    .and_then(|v| v.as_str())
            ...
            // Safety (2 tools)
            // ----------------------------------------------------------------
            "forge_safety" => self.client.get("/v1/forge/safety").await,
            "forge_kill_switch" => {
                let activate = obj
                    .get("activate")
            ...
            "forge_invoice" => {
                let cu_amount = obj
                    ...
                    .unwrap_or(0);
                self.client
                    .post("/v1/forge/invoice", serde_json::json!({"cu_amount": cu_amount}))
                    .await
            }
            "forge_route" => {
                ...
                let path = if params.is_empty() {
                    "/v1/forge/route".to_string()
                } else {
                    format!("/v1/forge/route?{}", params.join("&"))
                };
            ...
            // Lending (7 tools)
            // ----------------------------------------------------------------
            "forge_lend" => self.client.post("/v1/forge/lend", args).await,
            "forge_borrow" => self.client.post("/v1/forge/borrow", args).await,
            "forge_repay" => self.client.post("/v1/forge/repay", args).await,
            "forge_credit" => self.client.get("/v1/forge/credit").await,
            "forge_pool" => self.client.get("/v1/forge/pool").await,
            "forge_loans" => self.client.get("/v1/forge/loans").await,
            "forge_lend_to" => self.client.post("/v1/forge/lend-to", args).await,

            // ----------------------------------------------------------------
            // Bank L2 (8 tools)
            // ----------------------------------------------------------------
            "forge_bank_portfolio" => self.client.get("/v1/forge/bank/portfolio").await,
            "forge_bank_tick" => {
                self.client
                    .post("/v1/forge/bank/tick", serde_json::json!({}))
                    .await
            }
            "forge_bank_set_strategy" => {
                let strategy = obj
                    .get("strategy")
                    .and_then(|v| v.as_str())
                    .unwrap_or("balanced")
                ...
                self.client.post("/v1/forge/bank/strategy", body).await
            }
            "forge_bank_set_risk" => {
                let tolerance = obj
                    .get("tolerance")
                    ...
                    .to_string();
                self.client
                    .post("/v1/forge/bank/risk", serde_json::json!({"tolerance": tolerance}))
                    .await
            }
            "forge_bank_list_futures" => self.client.get("/v1/forge/bank/futures").await,
            "forge_bank_create_future" => {
                self.client.post("/v1/forge/bank/futures", args).await
            }
            "forge_bank_risk_assessment" => {
                self.client.get("/v1/forge/bank/risk-assessment").await
            }
            "forge_bank_optimize" => {
                self.client.post("/v1/forge/bank/optimize", args).await
            }

            // ----------------------------------------------------------------
            // Agora L4 (7 tools)
            // ----------------------------------------------------------------
            "forge_agora_register" => {
                self.client.post("/v1/forge/agora/register", args).await
            }
            "forge_agora_list_agents" => self.client.get("/v1/forge/agora/agents").await,
            "forge_agora_reputation" => {
                let hex = obj
                    .get("agent_hex")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string();
                self.client
                    .get(&format!("/v1/forge/agora/reputation/{hex}"))
                    .await
            }
            "forge_agora_find" => self.client.post("/v1/forge/agora/find", args).await,
            "forge_agora_stats" => self.client.get("/v1/forge/agora/stats").await,
            "forge_agora_snapshot" => self.client.get("/v1/forge/agora/snapshot").await,
            "forge_agora_restore" => {
                let snapshot = obj
                    .get("snapshot")
                    .cloned()
                ...
                self.client
                    .post("/v1/forge/agora/restore", snapshot)
                    .await
            }
            ...
            // Mind L3 (5 tools)
            // ----------------------------------------------------------------
            "forge_mind_init" => self.client.post("/v1/forge/mind/init", args).await,
            "forge_mind_state" => self.client.get("/v1/forge/mind/state").await,
            "forge_mind_improve" => {
            ...
            "forge_mind_budget" => self.client.post("/v1/forge/mind/budget", args).await,
            "forge_mind_stats" => self.client.get("/v1/forge/mind/stats").await,
...
mod tests {
    ...
    fn test_tool_groups_coverage() {
        ...
        // Bank L2
        for n in &[
            "forge_bank_portfolio",
            "forge_bank_tick",
            "forge_bank_set_strategy",
            "forge_bank_set_risk",
            "forge_bank_list_futures",
            "forge_bank_create_future",
            "forge_bank_risk_assessment",
            "forge_bank_optimize",
        ] {
        ...
        for n in &[
            "forge_agora_register",
            "forge_agora_list_agents",
            "forge_agora_reputation",
            "forge_agora_find",
            "forge_agora_stats",
            "forge_agora_snapshot",
            "forge_agora_restore",
        ] {
