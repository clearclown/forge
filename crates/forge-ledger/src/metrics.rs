...
//! - forge_pool_total_cu          (gauge, global)
...
impl ForgeMetrics {
    /// Create a new ForgeMetrics with its own isolated Registry.
    pub fn new() -> Self {
        ...
        let pool_total_cu = IntGauge::with_opts(Opts::new(
            "forge_pool_total_cu",
        ...
        registry
            .register(Box::new(pool_total_cu.clone()))
            .expect("register pool_total_cu");
        registry
    ...
    pub fn observe(&self, ledger: &ComputeLedger, now_ms: u64) {
        ...
        self.pool_total_cu.set(pool.total_pool_cu as i64);
        ...
        if !trades.is_empty() {
            ...
            let mut nodes: std::collections::HashSet<forge_core::NodeId> =
