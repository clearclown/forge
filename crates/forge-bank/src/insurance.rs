...
//! upfront.
//!
//! Ported from the Python scaffold `forge_bank/insurance.py`.
...
impl InsurancePolicy {
    ...
    pub fn new(
        ...
        if coverage_cu < premium_cu {
            return Err(BankError::InvalidParameter(
                "coverage_cu must be >= premium_cu".into(),
            ));
        }
...
pub fn premium_for(
    ...
    let raw = (coverage_cu as f64 * rate).floor() as u64;
    Ok(raw.max(1))
}

/// `premium_for` with default rates (base=0.02, risk=0.10).
pub fn premium_for_default(coverage_cu: u64, borrower_credit_score: f64) -> Result<u64, BankError> {
    premium_for(coverage_cu, borrower_credit_score, 0.02, 0.10)
}
...
mod tests {
    ...
    fn make_policy(coverage_cu: u64, premium_cu: u64) -> InsurancePolicy {
        InsurancePolicy::new(
            "p1",
            hex64("a"),
            hex64("b"),
