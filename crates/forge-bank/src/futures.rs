...
//!
//! Ported from the Python scaffold `forge_bank/futures.py`.
...
impl FuturesContract {
    ...
    pub fn new(
        ...
        if notional_cu == 0 {
            return Err(BankError::InvalidParameter("notional_cu must be > 0".into()));
        }
...
pub fn required_margin(contract: &FuturesContract, margin_fraction: f64) -> Result<u64, BankError> {
    ...
    Ok((contract.notional_cu as f64 * margin_fraction).floor() as u64)
}
