...
impl ForgeClient {
    ...
    /// `GET /v1/forge/balance` — CU balance: contributed, consumed, reserved,
    /// effective_balance, reputation.
    pub async fn balance(&self) -> Result<Balance, SdkError> {
        let v = self.get("/v1/forge/balance").await?;
        Ok(serde_json::from_value(v)?)
    }

    /// `GET /v1/forge/pricing` — Market price: cu_per_token, supply/demand
    /// factors, deflation, cost estimates.
    pub async fn pricing(&self) -> Result<Pricing, SdkError> {
        let v = self.get("/v1/forge/pricing").await?;
        Ok(serde_json::from_value(v)?)
    }

    /// `GET /v1/forge/trades?limit=N` — Recent trade history.
    pub async fn trades(&self, limit: u32) -> Result<serde_json::Value, SdkError> {
        self.get(&format!("/v1/forge/trades?limit={}", limit)).await
    }

    /// `GET /v1/forge/network` — Mesh economic summary with Merkle root.
    pub async fn network(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/network").await
    }

    /// `GET /v1/forge/providers` — Providers ranked by reputation and cost.
    pub async fn providers(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/providers").await
    }
    ...
    pub async fn can_afford(&self, estimated_tokens: u64) -> Result<bool, SdkError> {
        ...
        let cost = (pricing.cu_per_token * estimated_tokens as f64) as i64;
    ...
    /// `GET /v1/forge/safety` — Kill switch, circuit breaker, budget policy status.
    pub async fn safety(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/safety").await
    }

    /// `POST /v1/forge/kill` with `activate: true` — EMERGENCY: freeze all CU
    /// transactions.
    ...
    /// `POST /v1/forge/kill` with `activate: false` — Resume normal CU
    /// transactions.
    pub async fn resume(&self) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/kill", &serde_json::json!({"activate": false}))
            .await
    }
    ...
    /// `POST /v1/forge/invoice` — Create a Lightning invoice to convert CU to
    /// Bitcoin.
    pub async fn invoice(&self, cu_amount: u64) -> Result<serde_json::Value, SdkError> {
        self.post(
            "/v1/forge/invoice",
            &serde_json::json!({"cu_amount": cu_amount}),
        )
        .await
    }
    ...
    /// `POST /v1/forge/lend` — Contribute CU to the lending pool.
    pub async fn lend(
        &self,
        amount: u64,
        max_term_hours: u64,
        ...
        self.post("/v1/forge/lend", &body).await
    }
    ...
    pub async fn borrow(
        ...
        self.post("/v1/forge/borrow", &body).await
    }

    /// `POST /v1/forge/repay` — Repay an outstanding loan by loan_id.
    pub async fn repay(&self, loan_id: &str) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/repay", &serde_json::json!({"loan_id": loan_id}))
            .await
    }

    /// `GET /v1/forge/credit` — Credit score and component breakdown for this
    /// node.
    pub async fn credit(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/credit").await
    }

    /// `GET /v1/forge/pool` — Lending pool status and this node's max borrow
    /// capacity.
    pub async fn pool(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/pool").await
    }

    /// `GET /v1/forge/loans` — Active loans where this node is lender or
    /// borrower.
    pub async fn loans(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/loans").await
    }

    /// `POST /v1/forge/lend-to` — Lender-initiated loan proposal to a specific
    /// borrower.
    pub async fn lend_to(
        ...
        self.post("/v1/forge/lend-to", &body).await
    }
    ...
    /// `GET /v1/forge/route` — Find the optimal inference provider for a
    /// request.
    ///
    ...
    pub async fn route(
        ...
        let mut path = format!("/v1/forge/route?mode={}", mode);
        if let Some(m) = model {
    ...
    /// `GET /v1/forge/bank/portfolio` — PortfolioManager state.
    pub async fn bank_portfolio(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/bank/portfolio").await
    }

    /// `POST /v1/forge/bank/tick` — Run one PortfolioManager tick cycle.
    pub async fn bank_tick(&self) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/bank/tick", &serde_json::json!({})).await
    }

    /// `POST /v1/forge/bank/strategy` — Hot-swap portfolio strategy.
    ///
    ...
    pub async fn bank_set_strategy(
        ...
        self.post("/v1/forge/bank/strategy", &body).await
    }

    /// `POST /v1/forge/bank/risk` — Set risk tolerance.
    ///
    ...
    /// `GET /v1/forge/bank/futures` — List active futures contracts.
    pub async fn bank_list_futures(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/bank/futures").await
    }

    /// `POST /v1/forge/bank/futures` — Create a new CU futures contract.
    pub async fn bank_create_future(
        ...
        self.post("/v1/forge/bank/futures", &body).await
    }

    /// `GET /v1/forge/bank/risk-assessment` — Portfolio VaR, concentration,
    /// leverage.
    pub async fn bank_risk_assessment(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/bank/risk-assessment").await
    }

    /// `POST /v1/forge/bank/optimize` — Run YieldOptimizer with a VaR budget.
    pub async fn bank_optimize(&self, max_var_99_cu: u64) -> Result<serde_json::Value, SdkError> {
        self.post(
            "/v1/forge/bank/optimize",
    ...
    /// `POST /v1/forge/agora/register` — Register an agent in the Agora
    /// marketplace.
    ///
    ...
    pub async fn agora_register(
        ...
        self.post("/v1/forge/agora/register", &body).await
    }

    /// `GET /v1/forge/agora/agents` — List all registered AgentProfiles.
    pub async fn agora_list_agents(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/agora/agents").await
    }

    /// `GET /v1/forge/agora/reputation/{hex}` — Reputation score for a specific
    /// agent.
    pub async fn agora_reputation(&self, agent_hex: &str) -> Result<serde_json::Value, SdkError> {
        self.get(&format!("/v1/forge/agora/reputation/{}", agent_hex))
            .await
    }

    /// `POST /v1/forge/agora/find` — Find agents matching model patterns and
    /// optional filters.
    pub async fn agora_find(
        &self,
        model_patterns: &[&str],
        max_cu_per_token: Option<u64>,
        tier: Option<&str>,
        min_reputation: Option<f64>,
    ) -> Result<serde_json::Value, SdkError> {
        let mut body = serde_json::json!({"model_patterns": model_patterns});
        if let Some(m) = max_cu_per_token {
            body["max_cu_per_token"] = serde_json::json!(m);
        }
        if let Some(t) = tier {
        ...
        self.post("/v1/forge/agora/find", &body).await
    }

    /// `GET /v1/forge/agora/stats` — Agora registry statistics.
    pub async fn agora_stats(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/agora/stats").await
    }

    /// `GET /v1/forge/agora/snapshot` — Export a RegistrySnapshot for backup or
    /// migration.
    pub async fn agora_snapshot(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/agora/snapshot").await
    }

    /// `POST /v1/forge/agora/restore` — Restore the Agora registry from a
    /// RegistrySnapshot.
    pub async fn agora_restore(
        &self,
        snapshot: &serde_json::Value,
    ) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/agora/restore", snapshot).await
    }
    ...
    /// `POST /v1/forge/mind/init` — Initialise the ForgeMindAgent.
    ///
    ...
    pub async fn mind_init(
        ...
        self.post("/v1/forge/mind/init", &body).await
    }

    /// `GET /v1/forge/mind/state` — Current ForgeMindAgent state.
    pub async fn mind_state(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/mind/state").await
    }

    /// `POST /v1/forge/mind/improve` — Run N self-improvement cycles.
    pub async fn mind_improve(&self, n_cycles: usize) -> Result<serde_json::Value, SdkError> {
        self.post(
            "/v1/forge/mind/improve",
            &serde_json::json!({"n_cycles": n_cycles}),
        )
        .await
    }

    /// `POST /v1/forge/mind/budget` — Update ForgeMindAgent CU budget limits.
    /// Omit a field to leave it unchanged.
    pub async fn mind_budget(
        &self,
        max_cu_per_cycle: Option<u64>,
        max_cu_per_day: Option<u64>,
        max_cycles_per_day: Option<u64>,
    ) -> Result<serde_json::Value, SdkError> {
        let mut body = serde_json::json!({});
        if let Some(v) = max_cu_per_cycle {
            body["max_cu_per_cycle"] = serde_json::json!(v);
        }
        if let Some(v) = max_cu_per_day {
            body["max_cu_per_day"] = serde_json::json!(v);
        }
        if let Some(v) = max_cycles_per_day {
            body["max_cycles_per_day"] = serde_json::json!(v);
        }
        self.post("/v1/forge/mind/budget", &body).await
    }

    /// `GET /v1/forge/mind/stats` — ForgeMindAgent lifetime stats.
    pub async fn mind_stats(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/mind/stats").await
    }
    ...
    /// `GET /v1/forge/anchor?network=X` — Bitcoin OP_RETURN anchor status.
    pub async fn anchor(&self, network: &str) -> Result<serde_json::Value, SdkError> {
        self.get(&format!("/v1/forge/anchor?network={}", network))
            .await
    }

    /// `GET /v1/forge/collusion/{hex}` — Collusion resistance report for a peer.
    pub async fn collusion(&self, hex: &str) -> Result<serde_json::Value, SdkError> {
        self.get(&format!("/v1/forge/collusion/{}", hex)).await
    }

    /// `GET /v1/forge/reputation-gossip-status` — Reputation gossip debug info.
    pub async fn reputation_gossip_status(&self) -> Result<serde_json::Value, SdkError> {
        self.get("/v1/forge/reputation-gossip-status").await
    }

    /// `POST /v1/forge/admin/save-state` — Trigger manual ledger state
    /// persistence.
    pub async fn save_state(&self) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/admin/save-state", &serde_json::json!({}))
            .await
    }
    ...
    /// `POST /v1/forge/policy` — Set a budget policy on the safety controller.
    pub async fn set_policy(&self, body: &serde_json::Value) -> Result<serde_json::Value, SdkError> {
        self.post("/v1/forge/policy", body).await
    }
...
///
/// Wraps a `ForgeClient` and exposes high-level budget-aware methods.
///
/// ```rust,no_run
...
impl ForgeAgent {
    ...
    pub fn new(
        ...
        Self {
            client: ForgeClient::new(base_url, token),
            max_cu_per_task,
            min_balance,
            total_spent: 0,
        }
    ...
    pub async fn think(
        ...
        if let Some(ref forge) = result.x_forge {
            self.total_spent += forge.cu_cost;
        }
...
mod tests {
    ...
    fn test_client_new_trims_trailing_slash() {
        let c = ForgeClient::new("http://localhost:3000/", None);
        assert_eq!(c.base_url, "http://localhost:3000");
    }

    #[test]
    fn test_client_new_preserves_token() {
        let c = ForgeClient::new("http://localhost:3000", Some("tok"));
    ...
    fn test_pricing_response_deserializes() {
        let json = r#"{"cu_per_token":1.0,"supply_factor":1.0,"demand_factor":1.0,"cu_purchasing_power":1.0,"deflation_factor":1.0,"total_trades_ever":0,"estimated_cost_100_tokens":100,"estimated_cost_1000_tokens":1000}"#;
    ...
    fn test_pricing_response_deserializes_without_optional_field() {
        ...
        let json = r#"{"cu_per_token":2.0,"supply_factor":0.9,"demand_factor":1.1,"deflation_factor":0.99,"total_trades_ever":42,"estimated_cost_100_tokens":200,"estimated_cost_1000_tokens":2000}"#;
        let p: Pricing = serde_json::from_str(json).unwrap();
    ...
    fn test_chat_completion_deserializes() {
        let json = r#"{"id":"chatcmpl-x","object":"chat.completion","created":0,"model":"smollm2","choices":[],"usage":{},"x_forge":{"cu_cost":5,"effective_balance":1005}}"#;
        let c: ChatCompletion = serde_json::from_str(json).unwrap();
        assert_eq!(c.id, "chatcmpl-x");
        let forge = c.x_forge.unwrap();
        assert_eq!(forge.cu_cost, 5);
        assert_eq!(forge.effective_balance, 1005);
    }

    #[test]
    fn test_chat_completion_without_x_forge() {
        let json = r#"{"id":"chatcmpl-y","object":"chat.completion","created":0,"model":"llama","choices":[],"usage":{}}"#;
        let c: ChatCompletion = serde_json::from_str(json).unwrap();
        assert!(c.x_forge.is_none());
    }
    ...
    fn test_forge_agent_new() {
        let agent = ForgeAgent::new("http://127.0.0.1:3000", Some("secret"), 1000, 50);
