use forge_core::{ForgeError, ModelId, ModelManifest};
...
pub fn parse_gguf_metadata(path: &Path) -> Result<ModelManifest, ForgeError> {
    ...
    file.read_exact(&mut magic)
        .map_err(|e| ForgeError::ModelLoadError(format!("read magic: {e}")))?;
    if &magic != b"GGUF" {
        return Err(ForgeError::ModelLoadError("not a GGUF file".to_string()));
    }
...
fn read_u32_le(r: &mut impl Read) -> Result<u32, ForgeError> {
    let mut buf = [0u8; 4];
    r.read_exact(&mut buf)
        .map_err(|e| ForgeError::ModelLoadError(format!("read u32: {e}")))?;
    Ok(u32::from_le_bytes(buf))
}

fn read_u64_le(r: &mut impl Read) -> Result<u64, ForgeError> {
    let mut buf = [0u8; 8];
    r.read_exact(&mut buf)
        .map_err(|e| ForgeError::ModelLoadError(format!("read u64: {e}")))?;
    Ok(u64::from_le_bytes(buf))
}

fn read_i32_le(r: &mut impl Read) -> Result<i32, ForgeError> {
    let mut buf = [0u8; 4];
    r.read_exact(&mut buf)
        .map_err(|e| ForgeError::ModelLoadError(format!("read i32: {e}")))?;
    Ok(i32::from_le_bytes(buf))
}

fn read_string(r: &mut impl Read) -> Result<String, ForgeError> {
    let len = read_u64_le(r)? as usize;
    if len > 1024 * 1024 {
        return Err(ForgeError::ModelLoadError("string too long".to_string()));
    }
    let mut buf = vec![0u8; len];
    r.read_exact(&mut buf)
        .map_err(|e| ForgeError::ModelLoadError(format!("read string: {e}")))?;
