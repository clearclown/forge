...
//!
//! Counterpart to `forge_mind::cycle::ImprovementCycleRunner` — where the
...
impl GradientContribution {
    ...
    pub fn efficiency(&self) -> f64 {
        if self.compute_cost_cu == 0 {
            0.0
        } else {
            self.loss_delta() / self.compute_cost_cu as f64
        }
    }
}
